...
#include <PiDxe.h>
#include <Uefi.h>
#include <Protocol/Utc/Tegra210/Tegra.h>
#include <Protocol/Utc/Tegra210/Clock.h>
...
static INT8 periph_id_to_internal_id[PERIPH_ID_COUNT] = {
	/* Low word: 31:0 */
	NONE(CPU),
	NONE(COP),
	NONE(TRIGSYS),
...
{
    // Not expect to call in UEFI context
    // Code kept for reference purpose
    ASSERT(FALSE);
}
...
static int tegra_pllref_enable(void)
{
	...
	start = timer_get_us();
	while (timer_get_us() - start < 250) 
    {
		value = readl(NV_PA_CLK_RST_BASE + PLLREFE_MISC);
		if (value & PLLREFE_MISC_LOCK) break;
	}
...
struct periph_clk_init periph_clk_init_table[] = {
	...
	{ PERIPH_ID_DISP1, CLOCK_ID_DISPLAY },
