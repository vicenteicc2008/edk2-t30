...
function _help(){
	...
	echo
	echo "Build edk2 for Nvidia Tegra 3 platform."
	echo
	echo "Options: "
	...
	echo
	echo "MainPage: https://github.com/Vicc2008/edk2-t30"
	exit "${1}"
}
...
function _build(){
	...
	if "${GEN_ACPI}" && ! iasl -ve "Tegra3Pkg/AcpiTables/${DEVICE}/Dsdt.asl"
	...
	build -s -n 0 -a ARM -t GCC5 -p "Tegra3Pkg/Devices/${DEVICE}.dsc"||return "$?"
	gzip -c < workspace/Build/Tegra3Pkg/DEBUG_GCC5/FV/TEGRA3PKG_UEFI.fd > "workspace/uefi-${DEVICE}.img.gz"||return "$?"
...
cd "$(dirname "$0")"||exit 1
[ -f Tegra3Pkg/Tegra3Pkg.dsc ]||_error "cannot find Tegra3Pkg/Tegra3Pkg.dsc"
typeset -l DEVICE
DEVICE=""
CHINESE=false
CLEAN=false
